...
#define TEST_DEVICE_NAME            "ESP_MPPT"
...
static esp_attr_value_t gatts_demo_char1_val = { .attr_max_len =
		GATTS_DEMO_CHAR_VAL_LEN_MAX, .attr_len = sizeof(char1_str),
...
static esp_ble_adv_data_t adv_data = { .set_scan_rsp = false, .include_name =
		true, .include_txpower = false, .min_interval = 0x0006, //slave connection min interval, Time = min_interval * 1.25 msec
...
static esp_ble_adv_data_t scan_rsp_data = { .set_scan_rsp = true,
		.include_name = true, .include_txpower = true,
		//.min_interval = 0x0006,
		//.max_interval = 0x0010,
		.appearance = 0x00, .manufacturer_len = 0, //TEST_MANUFACTURER_DATA_LEN,
		.p_manufacturer_data = NULL, //&test_manufacturer[0],
...
static esp_gatt_if_t gatts_if_notify = ESP_GATT_IF_NONE;
		...
		if (!param->write.is_prep) {
			...
			if (gl_profile_tab[PROFILE_A_APP_ID].descr_handle
				...
				if (descr_value == 0x0001) {
					...
					gatts_if_notify = ESP_GATT_IF_NONE;
				} else {
			...
			else if(gl_profile_tab[PROFILE_A_APP_ID].char_handle
					== param->write.handle && param->write.len >= 1)
			{
				switch(param->write.value[0])
				{
				case 0x00:
				{
					uint32_t val = (((uint32_t)(param->write.value[3]))<<16) |
...
void send_notify(const measurement_t *meas) {
	if(gatts_if_notify == ESP_GATT_IF_NONE)
		return;

	uint8_t buff[sizeof(*meas)];
	memcpy(buff,meas,sizeof(*meas));

	//the size of notify_data[] need less than MTU size
	esp_ble_gatts_send_indicate(gatts_if_notify, notify_conn_id,
			gl_profile_tab[PROFILE_A_APP_ID].char_handle, sizeof(*meas),
			buff, false);
